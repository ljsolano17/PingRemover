...
class Ping{
	constructor(mod) {
		...
		this.mod = mod;
		this.command = mod.require.command;
		this.player = mod.require.library.player;

		this.setup();
	}

	setup = () => {
		const { mod, command } = this;
	...
	set_jitter = (skill_id, stage=null) => {
		const key = stage === null ? "end" : "stage";

		if(this.ping === 0) {
			this.setup_old_ping_detection(true);
		}

		const data = this._faked[key][skill_id];
		const jitter = Date.now() - (stage === null ? data : (data || {})[stage]) - this.ping;

		if(0 <= jitter && jitter <= user_config['maxAcceptedJitter']) {
			this.jitter = jitter;
	...
	add_to_faked = (skillId, stage=null) => {
		if(stage !== null) {
	...
	s_action_stage = (e, fake) => {
		...
		if(fake) this.add_to_faked(e.skill.id, e.stage);
		else this.set_jitter(e.skill.id, e.stage);
	}
	...
	s_action_end = (e, fake) => {
		...
		this.fix_event(e, fake);
		if(fake) this.add_to_faked(e.skill.id);
		else this.set_jitter(e.skill.id);
	}
